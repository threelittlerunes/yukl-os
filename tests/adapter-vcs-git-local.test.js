...
const RED_NOISY = "node -e \"console.error('boom-diagnostic'); process.exit(2)\"";
 ...
 * commit ahead. `main` is never checked out: a `scratch` branch is the
 * current branch, so a merge into `main` does not move a checked-out ref.
 */
async function withRepo(fn) {
  return withTempDir(async (dir) => {
    ...
    git(["checkout", "-q", "-b", "scratch", "main"], dir);
    return fn(dir);
  });
}
...
test("checks reports a failing exit code and is not ok", async () => {
  await withRepo(async (dir) => {
    ...
    assert.deepEqual(
      result.results.map(({ command, exitCode }) => ({ command, exitCode })),
      [
        { command: GREEN, exitCode: 0 },
        { command: RED_THREE, exitCode: 3 },
      ],
    );
    assert.equal(result.results[0].stderrTail, undefined);
  });
});

test("checks keeps the stderr tail on a failing command", async () => {
  await withRepo(async (dir) => {
    const vcs = createVcs({ repoDir: dir, commands: [GREEN, RED_NOISY] });
    const result = await vcs.checks("feature");
    assert.equal(result.ok, false);
    assert.equal(result.results[0].stderrTail, undefined);
    assert.match(result.results[1].stderrTail, /boom-diagnostic/);
  });
});
...
test("merge refuses a base checked out in a worktree and leaves base unchanged", async () => {
  await withRepo(async (dir) => {
    git(["worktree", "add", "-q", join(dir, "base-checkout"), "main"], dir);
    const vcs = createVcs({ repoDir: dir, commands: [GREEN] });
    const before = git(["rev-parse", "main"], dir).stdout.trim();
    ...
    assert.equal(result.ok, false);
    assert.equal(Object.hasOwn(result, "sha"), false);
    assert.match(result.error, /checked out/i);
    assert.equal(git(["rev-parse", "main"], dir).stdout.trim(), before);
  });
});
